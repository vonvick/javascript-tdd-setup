...
module.exports = function(author) {
  ...
  this.edit = function(note_id, new_content) {
    ...
    if (typeof note_id === 'string') {
    	return "Enter a number for the note ID"
    } else if(typeof note_id === 'number') {
        if (this.notes.length < note_id - 1 || note_id < 0) {
	         return 'Could not find the file to edit';
         } else {
	          this.notes[note_id - 1].note = new_content;
	          editResult += this.notes[note_id - 1].note;
	          return editResult;
	       }
    } else if(note_id && note_content === "") {
